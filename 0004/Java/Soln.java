class Solution {
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        */
        ...
        // Answer is guranteed, but calculations are complex outside loop hence low <= high.
        while(low <= high){
            ...
            */
            ...
            int maxLower1 = (p1 == 0) ? INT_MIN : nums1[p1-1];
            int minHigher1 = (p1 == s1) ? INT_MAX : nums1[p1];

            int maxLower2 = (p2 == 0) ? INT_MIN : nums2[p2-1];
            int minHigher2 = (p2 == s2) ? INT_MAX : nums2[p2];
            ...
            if( maxLower1 <= minHigher2 && maxLower2 <= minHigher1){
                ...
                if((s1 + s2) %2 == 0){
                    /* Total elements are even in number hence we need average of :-
                       ...
                       2. Minimum element in the right sub half of combined (minimum(minHigher1, minHigher2) )
                    */
                    return 0.5 * (Math.max(maxLower1, maxLower2) + Math.min(minHigher1,minHigher2));
                } else {
                    /* Total elements are odd in number hence we need the exact middle element.
                       Since the left sub half has one more element, the answer is maximum(maxLower1, maxLower2)
                    */
                    return Math.max(maxLower1, maxLower2);
                }
            } else if(maxLower1 > minHigher2){
